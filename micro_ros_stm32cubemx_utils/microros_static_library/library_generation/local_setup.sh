#!/bin/bash
set -e
export MICROROS_LIBRARY_FOLDER=$(realpath "$(dirname "$0")/../..")
export BASE_PATH="$MICROROS_LIBRARY_FOLDER/microros_static_library"
...
pushd firmware/mcu_ws > /dev/null
    ...
    pushd extra_packages > /dev/null
    	USER_CUSTOM_PACKAGES_DIR=$BASE_PATH/library_generation/extra_packages 
...
pushd $MICROROS_LIBRARY_FOLDER/.. > /dev/null
